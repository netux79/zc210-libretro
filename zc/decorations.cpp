...
dBushLeaves::dBushLeaves(fix X, fix Y, int Id, int Clk) : decoration(X, Y, Id,
         Clk)
{
   ox = X;
   oy = Y;
...
dFlowerClippings::dFlowerClippings(fix X, fix Y, int Id,
                                   int Clk) : decoration(X, Y, Id, Clk)
{
   ox = X;
   oy = Y;
   id = Id;
...
dGrassClippings::dGrassClippings(fix X, fix Y, int Id, int Clk) : decoration(X,
         Y, Id, Clk)
{
   ox = X;
   oy = Y;
...
dHammerSmack::dHammerSmack(fix X, fix Y, int Id, int Clk) : decoration(X, Y, Id,
         Clk)
{
   ox = X;
   oy = Y;
...
dTallGrass::dTallGrass(fix X, fix Y, int Id, int Clk) : decoration(X, Y, Id,
         Clk)
{
   id = Id;
   clk = Clk;
...
dNayrusLoveShield::dNayrusLoveShield(fix X, fix Y, int Id,
                                     int Clk) : decoration(X, Y, Id, Clk)
{
   id = Id;
   clk = Clk;
...
void dNayrusLoveShield::realdraw(BITMAP *dest, int draw_what)
{
   ...
   flip = 0;
   if (((LinkNayrusLoveShieldClk() & 0x20) || (LinkNayrusLoveShieldClk() & 0xF00))
         && ((!get_bit(quest_rules, qr_FLICKERINGNAYRUSLOVESHIELD))
             || ((misc == 1) ? (frame & 1) : (!(frame & 1)))))
   {
