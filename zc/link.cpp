...
{
   lstep = lstep < (BSZ ? 27 : 11) ? lstep + 1 : 0;
}
   ...
   if (ladderx + laddery)
   {
      ...
      if ((ladderdir >= left) && (get_bit(quest_rules, qr_RLFIX)))
      {
         overtile16(dest, itemsbuf[iLadder].tile, ladderx, laddery + 56,
                    itemsbuf[iLadder].csets & 15,
                    rotate_value((itemsbuf[iRaft].misc >> 2) & 3) ^ 3);
      }
      else
      {
         overtile16(dest, itemsbuf[iLadder].tile, ladderx, laddery + 56,
...
void LinkClass::draw(BITMAP *dest)
{
   ...
   yofs = (!BSZ && isdungeon() && currscr < 128
           && !get_bit(quest_rules, qr_LINKDUNGEONPOSFIX)) ? 54 : 56;
   if (action != dying)
   {
      cs = 6;
   ...
   if (action == holding1 || action == holding2)
   {
      ...
      yofs = (!BSZ && isdungeon() && currscr < 128
              && !get_bit(quest_rules, qr_LINKDUNGEONPOSFIX)) ? 54 : 56;
      linktile(&tile, &flip, (action == holding1) ? ls_hold1 : ls_hold2, dir,
               zinit.linkwalkstyle);
      if (get_bit(quest_rules, qr_HOLDITEMANIMATION))
         putitem2(dest, x - ((action == holding1) ? 4 : 0), y + yofs - 16, holditem,
   ...
   {
      useltm = get_bit(quest_rules, qr_FULLLTM);
      linktile(&tile, &flip, (action == swimhold1) ? ls_swimhold1 : ls_swimhold2, dir,
               zinit.linkwalkstyle);
      if (get_bit(quest_rules, qr_HOLDITEMANIMATION))
         putitem2(dest, x - ((action == swimhold1) ? 4 : 0), y + yofs - 12, holditem,
                  lens_hint_item[holditem][0], lens_hint_item[holditem][1], 0);
...
{
   if (isdungeon() && currscr < 128 && (x < 16 || x > 224 || y < 18 || y > 146)
...
{
   if (action != attacking || (attack != wSword && attack != wWand
                               && attack != wHammer)
         || (attackclk <= 4))
      return;
   ...
   {
      if (guys.spr(i)->hit(wx, wy, wxsz, wysz) || (attack == wWand
            && guys.spr(i)->hit(x, y - 8, 16, 24))
   ...
   if (!get_bit(quest_rules, qr_NOITEMMELEE))
   {
      for (int j = 0; j < items.Count(); j++)
      {
         if (((item *)items.spr(j))->pickup & ipTIMER)
         {
            ...
            {
               if (items.spr(j)->hit(wx, wy, wxsz, wysz) || (attack == wWand
                     && items.spr(j)->hit(x, y - 8, 16, 24))
...
int LinkClass::EwpnHit()
{
   for (int i = 0; i < Ewpns.Count(); i++)
      if (Ewpns.spr(i)->hit(x + 7, y + 7, 2, 2))
      {
         ...
         int oldid = ew->id;
         ew->onhit(false, (current_item(itype_shield, true) >= i_mirrorshield) ? 2 : 1,
                   dir);
         if (ew->id !=
               oldid)                                    // changed type from ewX to wX
         {
...
int LinkClass::LwpnHit()                                    //only here to check magic hits
{
   for (int i = 0; i < Lwpns.Count(); i++)
      if (Lwpns.spr(i)->hit(x + 7, y + 7, 2, 2))
      {
         ...
         lw->onhit(false, (current_item(itype_shield, true) >= i_mirrorshield) ? 2 : 1,
                   dir);
         if (lw->id == wRefMagic)
         {
            lw->ignoreLink = true;
...
void LinkClass::checkhit()
{
   ...
   {
      if (((ladderx + laddery) && ((hitdir & 2) == ladderdir))
            || (!(ladderx + laddery)))
      {
         for (int i = 0; i < 4; i++)
         {
   ...
   for (int i = 0; i < Lwpns.Count(); i++)
   {
      ...
      if (!get_bit(quest_rules, qr_FIREPROOFLINK))
      {
         ...
         {
            if (NayrusLoveShieldClk <= 0)
               game.life = max(game.life - ((HP_PER_HEART / 2) >> current_item(itype_ring,
                                            true)), 0);
      ...
      if (get_bit(quest_rules, qr_OUCHBOMBS))
      {
         ...
         if (((s->id == wBomb) || (s->id == wSBomb)) && s->hit(this) && !superman)
         {
            if (NayrusLoveShieldClk <= 0)
            {
               game.life = max(game.life - ((4 * HP_PER_HEART / 2) >> current_item(itype_ring,
                                            true)), 0);
               if (s->id == wSBomb)
                  game.life = max(game.life - ((12 * HP_PER_HEART / 2) >> current_item(itype_ring,
                                               true)), 0);
            }
            hitdir = s->hitdir(x, y, 16, 16, dir);
            if (action != rafting && action != freeze)
   ...
   {
      if (NayrusLoveShieldClk <= 0)
         game.life = max(game.life - (lwpn_dp(hit) >> current_item(itype_ring, true)),
                         0);
   ...
   {
      if (NayrusLoveShieldClk <= 0)
         game.life = max(game.life - (ewpn_dp(hit) >> current_item(itype_ring, true)),
                         0);
   ...
   {
      if ((!(current_item(itype_boots, true)))
            || ((get_bit(quest_rules, qr_MAGICBOOTS))
...
void LinkClass::hitlink(int hit)
{
   ...
   if (NayrusLoveShieldClk <= 0)
      game.life = max(game.life - (enemy_dp(hit) >> current_item(itype_ring, true)),
                      0);
...
bool LinkClass::animate(int index)
{
   ...
   if ((COMBOTYPE(x, y + 15) == cTALLGRASS)
         && (COMBOTYPE(x + 15, y + 15) == cTALLGRASS))
   {
      if (decorations.idCount(dTALLGRASS) == 0)
         decorations.add(new dTallGrass(x, y, dTALLGRASS, 0));
   }

   if ((COMBOTYPE(x, y + 15) == cSHALLOWWATER)
         && (COMBOTYPE(x + 15, y + 15) == cSHALLOWWATER))
   {
   ...
   if (add_df1asparkle)
   {
      Lwpns.add(new weapon((fix)(df_x + (rand() % 7)), (fix)(df_y + (rand() % 7)),
                           wPhantom, pDINSFIREROCKETTRAIL, 0, 0));
   ...
   {
      Lwpns.add(new weapon((fix)(df_x + (rand() % 7)), (fix)(df_y + (rand() % 7)),
                           wPhantom, pDINSFIREROCKETTRAILRETURN, 0, 0));
      add_df1bsparkle = false;
   }

   if (add_nl1asparkle)
   {
      Lwpns.add(new weapon((fix)(nl1_x + (rand() % 7)), (fix)(nl1_y + (rand() % 7)),
                           wPhantom, pNAYRUSLOVEROCKETTRAIL1, 0, 0));
   ...
   {
      Lwpns.add(new weapon((fix)(nl1_x + (rand() % 7)), (fix)(nl1_y + (rand() % 7)),
                           wPhantom, pNAYRUSLOVEROCKETTRAILRETURN1, 0, 0));
      add_nl1bsparkle = false;
   }

   if (add_nl2asparkle)
   {
      Lwpns.add(new weapon((fix)(nl2_x + (rand() % 7)), (fix)(nl2_y + (rand() % 7)),
                           wPhantom, pNAYRUSLOVEROCKETTRAIL2, 0, 0));
   ...
   {
      Lwpns.add(new weapon((fix)(nl2_x + (rand() % 7)), (fix)(nl2_y + (rand() % 7)),
                           wPhantom, pNAYRUSLOVEROCKETTRAILRETURN2, 0, 0));
   ...
   {
      if (((dtype == dBOMBED) ? DrunkUp() : dir == up) && x == 120 && y <= 32
            && tmpscr->door[0] == dtype)
      {
         walk = true;
         dir = up;
      ...
      if (((dtype == dBOMBED) ? DrunkDown() : dir == down) && x == 120 && y >= 128
            && tmpscr->door[1] == dtype)
      {
         walk = true;
         dir = down;
      ...
      if (((dtype == dBOMBED) ? DrunkLeft() : dir == left) && x <= 32 && y == 80
            && tmpscr->door[2] == dtype)
      {
         walk = true;
         dir = left;
      ...
      if (((dtype == dBOMBED) ? DrunkRight() : dir == right) && x >= 208 && y == 80
            && tmpscr->door[3] == dtype)
      {
         walk = true;
         dir = right;
   ...
   switch (wpn)
   {
      ...
      case iRCandle:
         ...
         if ((get_bit(quest_rules, qr_ENABLEMAGIC)) &&
            ...
            return false;
         didstuff |= did_candle;
         Lwpns.add(new weapon((fix)wx, (fix)wy, wFire, (wpn == iBCandle) ? 0 : 1,
      ...
      case iFBrang:
         if (Lwpns.idCount(wBrang))
            return false;
         Lwpns.add(new weapon((fix)wx, (fix)wy, wBrang, current_item(itype_brang, true),
                              (current_item(itype_brang, true)*DAMAGE_MULTIPLIER), dir));
         break;
      ...
      case iHookshot:
         ...
         if (use_hookshot)
         {
            if (dir == up)
            {
               ...
               Lwpns.add(new weapon((fix)wx, (fix)wy - 4, wHookshot, 0, DAMAGE_MULTIPLIER,
                                    dir));
               hs_startx = wx;
               hs_starty = wy - 4;
            }
            ...
            if (dir == left)
            {
               hookshot_used = true;
               Lwpns.add(new weapon((fix)wx, (fix)wy, wHSHandle, 0, DAMAGE_MULTIPLIER, dir));
               Lwpns.add(new weapon((fix)(wx - 4), (fix)wy, wHookshot, 0, DAMAGE_MULTIPLIER,
                                    dir));
               hs_startx = wx - 4;
               hs_starty = wy;
            }
...
bool LinkClass::doattack()
{
   ...
   {
      if (DrunkUp() && !DrunkLeft() && !DrunkRight() && !DrunkDown())
         dir = up;
      if (!DrunkUp() && !DrunkLeft() && !DrunkRight() && DrunkDown())
         dir = down;
      if (!DrunkUp() && DrunkLeft() && !DrunkRight() && !DrunkDown())
...
void LinkClass::do_hopping()
{
   ...
   {
      if (dir < left ? !(int(x) & 7) && !(int(y) & 15) : !(int(x) & 15)
            && !(int(y) & 7))
      {
         action = none;
         hopclk = 0;
...
void LinkClass::movelink()
{
   ...
   if (can_attack() && current_item(itype_sword, true) > 0 && swordclk == 0
         && DrunkrAbtn())
   {
      action = attacking;
      attack = wSword;
...
bool LinkClass::walkflag(int wx, int wy, int cnt, byte d)
{
   ...
   if (isdungeon() && currscr < 128 && wy < 40 && (x != 120
         || _walkflag(120, 24, 2))
         && !get_bit(quest_rules, qr_FREEFORM))
      return true;
   ...
   else if (wf)
   {
      ...
      if (can_use_item(itype_ladder, i_ladder) && (tmpscr->flags & fLADDER
            || isdungeon()))
      {
         // add ladder combos
         ...
         if ((d >= left && wtrx) || (d <= down && ((wtrx && !flgx8) || (wtrx8
                                     && !flgx))))
         {
...
void LinkClass::checkpushblock()
{
   ...
   if ((t == cPUSH_WAIT || t == cPUSH_HW || t == cPUSH_HW2) && (pushing < 16
         || hasMainGuy()))
      return;
   if ((t == cPUSH_HEAVY || t == cPUSH_HW)
         && (current_item(itype_bracelet, true) < 1))
      return;
   if ((t == cPUSH_HEAVY2 || t == cPUSH_HW2)
         && (current_item(itype_bracelet, true) < 2))
      return;

   if (get_bit(quest_rules, qr_HESITANTPUSHBLOCKS) && (pushing < 4))
      return;
...
void LinkClass::checklocked()
{
   ...
   if (pushing != 8)
      return;
   if ((tmpscr->door[dir] != dLOCKED) && (tmpscr->door[dir] != dBOSS))
      return;
...
void LinkClass::checkspecial()
{
         ...
         type != cPIT && type != cSWIMWARP && !(type == cDIVEWARP && diveclk > 30))
   {
      switch (flag)
      {
         ...
         case mfRAFT:
         case mfRAFT_BRANCH:
            if ((current_item(itype_raft, true) >= i_raft) && action != rafting
                  && action != swimhit
...
bool LinkClass::dowarp(int type)
{
   ...
   switch (wtype)
   {
      case wtCAVE:                                            // cave/item room
         ...
         if (dlevel == 0)                                      // cave
         {
            ...
            if (tmpscr->room == rWARP)
               currscr = 0x81;
            loadlvlpal(10);
            blackscr(30, (COOLSCROLL && ((combobuf[MAPDATA(x, y - 16)].type == cCAVE)
                                         || (combobuf[MAPDATA(x, y - 16)].type == cCAVE2))) ? false : true);
      ...
      case wtEXIT:                                            // entrance/exit
         ...
         homescr = currscr = wscr + (((DMaps[currdmap].type & dmfTYPE) == dmOVERW) ? 0 :
                                     DMaps[currdmap].xoff);
         if (dlevel)
            lastentrance = currscr;

         else
            lastentrance = DMaps[currdmap].cont + (((DMaps[currdmap].type & dmfTYPE) ==
                                                    dmOVERW) ? 0 : DMaps[currdmap].xoff);
         loadscr(0, currscr, -1);
         if (dlevel)
         {
            x = tmpscr->warparrivalx;
      ...
      case wtSCROLL:                                          // scrolling warp
      {
         ...
         currdmap = wdmap;
         dlevel = DMaps[currdmap].level;
         homescr = currscr = wscr + (((DMaps[wdmap].type & dmfTYPE) == dmOVERW) ? 0 :
                                     DMaps[wdmap].xoff);
         if (!intradmap)
         {
      ...
      if (dlevel)
      {
         ...
         else
         {
            if (currdmap != game.continue_dmap)
               game.continue_scrn = DMaps[currdmap].cont + (((DMaps[currdmap].type & dmfTYPE)
                                    == dmOVERW) ? 0 : DMaps[currdmap].xoff);
         }
      }
      else
...
void LinkClass::exitcave()
{
   ...
   if (x + y == 0)
      x = y = 80;
   bool b = ((combobuf[MAPDATA(x, y - 16)].type == cCAVE)
             || (combobuf[MAPDATA(x, y - 16)].type == cCAVE2)) && COOLSCROLL;
...
void getitem(int id)
{
   switch (id)
   {
      ...
      // fall through
      case iHeartC:
         if (game.maxlife < (get_bit(quest_rules,
                                     qr_24HC) ? 24 * HP_PER_HEART : 16 * HP_PER_HEART))
...
void LinkClass::checkitems()
{
   ...
   if ((pickup & ipTIMER) && (((item *)items.spr(index))->clk2 < 32))
      if ((items.spr(index)->id != iFairyMoving)
            && (items.spr(index)->id != iFairyMoving))
         // wait for it to stop flashing, doesn't check for other items yet
         return;

   if (pickup &
         ipENEMY)                                     // item was being carried by enemy
      hasitem = 0;

   if (pickup &
         ipDUMMY)                                     // dummy item (usually a rupy)
   {
      if (pickup & ipMONEY)
...
void LinkClass::StartRefill(int refill_why)
{
   if (!refilling)
   {
      ...
      if ((refill_why == REFILL_POTION)
         ...
         swordclk = 0;
      if ((refill_why == REFILL_FAIRY)
            && (!get_bit(quest_rules, qr_NONBUBBLEFAIRIES)))
         swordclk = 0;
      if ((refill_why == REFILL_TRIFORCE)
            && (!get_bit(quest_rules, qr_NONBUBBLETRIFORCE)))
         swordclk = 0;
   }
}
...
void LinkClass::getTriforce(int id)
{
   PALETTE flash_pal;
   for (int i = 0; i < 256; i++)
      flash_pal[i] = get_bit(quest_rules, qr_FADE) ? _RGB(63, 63, 0) : _RGB(63, 63,
                     63);
...
void red_shift()
{
   ...
   for (int y = 0; y < 168; y++)
   {
      for (int x = 0; x < 256; x++)
      {
         ...
         int r = min(int(RAMpal[c].r * 0.4 + RAMpal[c].g * 0.6 + RAMpal[c].b * 0.4) >> 1,
                     31);
