...
void blueframe(BITMAP *dest, int x, int y, int xsize, int ysize)
{
   ...
   */
   ...
   {
      int top    = (dx >= (xsize >> 1)) ? ((dx == xsize - 1) ? t8 + 5  : t8 + 4) : ((
                      dx == 0) ? t8    : t8 + 1);
      int bottom = (dx >= (xsize >> 1)) ? ((dx == xsize - 1) ? t8 + 87 : t8 + 86) : ((
                      dx == 0) ? t8 + 82 : t8 + 83);
   ...
   for (int dy = 1; dy < ysize - 1; dy++)
   {
      ...
      {
         int fill = (dy >= (ysize >> 1)) ? ((dx >= (xsize >> 1)) ? t8 + 84 : t8 + 81)
                    : ((dx >= (xsize >> 1)) ? t8 + 6 : t8 + 3);
   ...
   switch ((DMaps[currdmap].type & dmfTYPE))
   {
      ...
      case dmBSOVERW:
         ...
         if (DMaps[currdmap].minimap_1_tile)
            draw_block(dest, 8, 8, DMaps[currdmap].minimap_1_tile,
                       DMaps[currdmap].minimap_1_cset, 5, 3);
...
void putsubscr(BITMAP *dest, int x, int y, bool showtime)
{
   ...
   {
      if (type == dmOVERW)
         putdot(subscr, ((homescr & 15) << 2) + 17, ((homescr & 0xF0) >> 2) + 16,
                QMisc.colors.link_dot);

      else if (type == dmBSOVERW || ((type == dmDNGN || type == dmCAVE)
                                     && currscr < 128))
         putdot(subscr, (((homescr & 15) - DMaps[currdmap].xoff) << 3) + 18,
                ((homescr & 0xF0) >> 2) + 16, QMisc.colors.link_dot);
   }
   ...
   if (showtime)
   {
      ...
      textout_ex(subscr, zfont, ts, 240 - (strlen(ts) << 3), 0, QMisc.colors.text,
                 -1);
   }
   ...
   textout_ex(subscr, zfont, "B", 128, 16, QMisc.colors.text,
              QMisc.colors.subscr_bg);
   textout_ex(subscr, zfont, "A", 152, 16, QMisc.colors.text,
              QMisc.colors.subscr_bg);
...
void puttriframe(BITMAP *dest)
{
   if (QMisc.colors.triframe_tile)
      draw_block(dest, 80, 104, QMisc.colors.triframe_tile,
                 QMisc.colors.triframe_cset, 6, 3);
   ...
   for (int i = 0; i < 8; i++)
   {
      ...
      if (game.lvlitems[lvl]&liTRIFORCE)
      {
         ...
         overtile16(dest, QMisc.colors.triforce_tile, tripiece[tp * 3],
                    tripiece[tp * 3 + 1], QMisc.colors.triforce_cset, tripiece[tp * 3 + 2]);
      }
   }
}
...
void putBmap(BITMAP *dest)
{
   ...
   if (currscr < 128)
      putdot(dest, (((homescr & 15) - DMaps[currdmap].xoff) << 3) + 130 +
...
void load_Sitems(void)
{
   ...
   if (current_item(itype_ring, true))
   {
      if (current_item(itype_ring, true) >= 3)
         ...
         Sitems.add(new item((fix)(x[2] - ofs), (fix)y, current_item(itype_ring,
                             true) - 1 + iBRing, 0, 0));
   }
      ...
      //  if(NEWSUBSCR)
   {
      ...
      if (current_item(itype_wallet, true))
         Sitems.add(new item((fix)(x[1] - ofs), (fix)y,
                             iWallet500 - 1 + current_item(itype_wallet, true), 0, 0));
   ...
   if (current_item(itype_brang, true))
   {
      if (current_item(itype_brang, true) < i_fbrang)
         Sitems.add(new item((fix)(128 + ofs), (fix)y, current_item(itype_brang,
                             true) - 1 + iBrang, 0, 0));
   ...
   if (current_item(itype_arrow, true))
   {
      if (current_item(itype_arrow, true) < i_garrow)
         Sitems.add(new item((fix)(172 + ofs), (fix)y, current_item(itype_arrow,
                             true) - 1 + iArrow, 0, 0));
   ...
   if (current_item(itype_candle, true))
      Sitems.add(new item((fix)(200 + ofs), (fix)y, current_item(itype_candle,
                          true) - 1 + iBCandle, 0, 0));
   ...
   if (current_item(itype_potion, true))
      Sitems.add(new item((fix)(176 + ofs), (fix)y, current_item(itype_potion,
                          true) - 1 + iBPotion, 0, 0));
