...
int enemy::takehit(int wpnId, int power, int wpnx, int wpny, int wpnDir)
{
   if (dying || clk < 0 || hclk > 0)
      return 0;
   if (superman && !(wpnId == wSBomb
                     && superman == 2)       // vulnerable to super bombs
...
bool enemy::canmove(int ndir, fix s, int special, int dx1, int dy1, int dx2,
                    int dy2)
{
   bool ok;
   switch (ndir)
   {
      case 8:
      case up:
         ok = !m_walkflag(x, y + dy1 - s,
                          (special == spw_clipbottomright) ? spw_none : special)
              && !((special == spw_floater) && (COMBOTYPE(x, y + dy1 - s) == cNOFLYZONE));
         break;
      case 12:
      case down:
         ok = !m_walkflag(x, y + dy2 + s, special) && !((special == spw_floater)
               && (COMBOTYPE(x, y + dy2 + s) == cNOFLYZONE));
         break;
      case 14:
      case left:
         ok = !m_walkflag(x + dx1 - s, y + 8, (special == spw_clipbottomright
                                               || special == spw_clipright) ? spw_none : special)
              && !((special == spw_floater) && (COMBOTYPE(x + dx1 - s, y + 8) == cNOFLYZONE));
         break;
      case 10:
      case right:
         ok = !m_walkflag(x + dx2 + s, y + 8, special) && !((special == spw_floater)
               && (COMBOTYPE(x + dx2 + s, y + 8) == cNOFLYZONE));
         break;
      case 9:
      case r_up:
         ok = !m_walkflag(x, y + dy1 - s, special)
              && !m_walkflag(x + dx2 + s, y + 8, special)
              && !((special == spw_floater) && (COMBOTYPE(x, y + dy1 - s) == cNOFLYZONE))
              && !((special == spw_floater) && (COMBOTYPE(x + dx2 + s, y + 8) == cNOFLYZONE));
         break;
      case 11:
      case r_down:
         ok = !m_walkflag(x, y + dy2 + s, special)
              && !m_walkflag(x + dx2 + s, y + 8, special)
              && !((special == spw_floater) && (COMBOTYPE(x, y + dy2 + s) == cNOFLYZONE))
              && !((special == spw_floater) && (COMBOTYPE(x + dx2 + s, y + 8) == cNOFLYZONE));
         break;
      case 13:
      case l_down:
         ok = !m_walkflag(x, y + dy2 + s, special)
              && !m_walkflag(x + dx1 - s, y + 8, special)
              && !((special == spw_floater) && (COMBOTYPE(x, y + dy2 + s) == cNOFLYZONE))
              && !((special == spw_floater) && (COMBOTYPE(x + dx1 - s, y + 8) == cNOFLYZONE));
         break;
      case 15:
...
void enemy::halting_walk(int rate, int homing, int special, int hrate,
                         int haltcnt)
{
   if (sclk && clk2)
      clk3 = 0;
...
void enemy::variable_walk_8(int rate, int newclk, int special, int dx1, int dy1,
                            int dx2, int dy2)
{
   if (clk < 0 || dying || stunclk || watch)
      return;
...
void enemy::floater_walk(int rate, int newclk, fix ms, fix ss, int s, int p,
                         int g)
{
   ++clk2;
   switch (movestatus)
...
{
   bool f2 = (clk >= (frate >>
                      1));                          // second frame of 2 frame animations
...
void eGhini::draw(BITMAP *dest)
{
   ...
   if (get_bit(quest_rules, qr_NEWENEMYTILES))
   {
      if (id == eGHINI2)
      {
         tile += 20;
         switch (dir -
                 8)                                      //directions get screwed up after 8.  *shrug*
         {
      ...
      switch (misc)
      {
         ...
         case 1:                                               // waiting to pounce
            if (--clk2 <= 0)
            {
               ...
               cstart = c = 9 - ((r & 31) >>
                                 3);                   // time before gravity kicks in
            }
            break;
...
bool eZora::animate(int index)
{
   ...
   switch (clk)
   {
      case 0:                                                 // reposition him
      {
         ...
         if (!placed
               || whistleclk >= 88)                      // can't place him, he's gone
            return true;
...
void eBubble::draw(BITMAP *dest)
{
   ...
   if (get_bit(quest_rules, qr_NEWENEMYTILES))
   {
      ...
      flip  = f2 ? 1 : 0;
   if (((id == eBUBBLE) || (id == eIBUBBLE))
         && (!get_bit(quest_rules, qr_NOBUBBLEFLASH)))
      cs = (frame & 3) + 6;
   enemy::draw(dest);
}
...
void ePolsVoice::polsvoice_hop()
{
   ...
   if (clk3 <= 0)
   {
      ...
      {
         fy = y;
         if (!canmove(dir, (fix)2, spw_none) || m_walkflag(x, y, spw_none)
               || !(rand() % 3))
            clk2 = 32;
      }
      clk3 = 32;
...
bool eLikeLike::animate(int index)
{
   ...
   if (haslink)
   {
      ...
      if ((clk & 0x18) ==
            8)                                  // stop its animation on the middle frame
         --clk;
   }
   else
...
skip1:
   ...
   {
      case wBrang:
         if ((current_item(itype_brang, true) < 3) || ((dir == up) && (wpny < y))
               || ((dir == down) && (wpny > y))
            ...
            case 0:
               ...
               else
               {
                  ...
                  while (!placed && t < 160)
                  {
                     ...
                     if ((!m_walkflag(x, y, 1)) && ((abs((int)(x - Link.getX())) >= 32)
                                                    || (abs((int)(y - Link.getY())) >= 32)))
                     {
                        //        if(iswater(tmpscr->data[pos]) && (pos&15)>0 && (pos&15)<15) {
                        //               x=(pos&15)<<4;
                        //               y=pos&0xF0;
...
void eWizzrobe::wizzrobe_attack()
{
   ...
   if (clk < 0 || dying || stunclk || watch)
      return;
   if (clk3 <= 0 || ((clk3 & 31) == 0 && !canmove(dir, (fix)1, spw_door)
                     && !misc))
   {
      fix_coords();
      switch (misc)
   ...
   if (get_bit(quest_rules, qr_NEWENEMYTILES))
   {
      ...
      if (d->misc1)                                           //walking wizzrobe
      {
         ...
         if (clk & 4)
            tile += 1;
         if (!(charging
               || firing))                            //should never be charging or firing for these wizzrobes
         {
            if (fclk > 0)
            {
               tile += 40;
...
int eDodongo::takehit(int wpnId, int power, int wpnx, int wpny, int wpnDir)
{
   ...
   switch (wpnId)
   {
      ...
      case wLitBomb:
      case wLitSBomb:
         if (abs((int)(wpnx - ((dir == right) ? x + 16 : x))) > 7
               || abs((int)(wpny - y)) > 7)
            return 0;
         clk2 = 96;
         misc = power;
      ...
      case wBomb:
      case wSBomb:
         if (abs((int)(wpnx - ((dir == right) ? x + 16 : x))) > 8
               || abs((int)(wpny - y)) > 8)
            return 0;
         stunclk = 160;
...
eAquamentus::eAquamentus(fix X, fix Y, int Id, int Clk) : enemy((fix)176,
         (fix)64, Id, Clk)
{
   if (id == eLAQUAM)
      x = 64;
...
void eAquamentus::draw(BITMAP *dest)
{
   if (get_bit(quest_rules, qr_NEWENEMYTILES))
   {
      ...
      tile = dummy_int[0] + ((clk & 24) >> 2) + (clk3 > -32 ?
             (clk3 > 0 ? 40 : 80) : 0);
      if (dying)
      {
         xofs = 0;
   ...
   else
   {
      ...
      tile = dummy_int[0] + ((clk3 > 0) ? 0 : 2);
      enemy::draw(dest);
      // tail (
      tile = dummy_int[0] + ((clk & 16) ? 1 : 3);
      xofs = xblockofs;
      enemy::draw(dest);
      // body
      yofs += 16;
      xofs = 0;
      tile = dummy_int[0] + ((clk & 16) ? 20 : 22);
      enemy::draw(dest);
      xofs = xblockofs;
      tile = dummy_int[0] + ((clk & 16) ? 21 : 23);
      enemy::draw(dest);
      yofs -= 16;
   }
}
...
int esMoldorm::takehit(int wpnId, int power, int wpnx, int wpny, int wpnDir)
{
   ...
   {
      return (wpnId == wSBomb) ? 1 :
             2;    // force it to wait a frame before checking sword attacks again
   }
   return 0;
}
...
eGleeok::eGleeok(fix X, fix Y, int Id, int Clk) : enemy((fix)120, (fix)48, Id,
         Clk)
{
   flameclk = 0;
...
bool eGleeok::animate(int index)
{
   ...
   if (hp == -1000)
   {
      for (int i = 0; i < clk3; ++i)
      {
         ...
         ((enemy *)guys.spr(index + i + 1))->animate(index + i +
               1); // let it animate one frame,
   ...
   {
      ((enemy *)guys.spr(index + clk3))->misc =
         -1;           // give signal to fly off
...
bool esGleeok::animate(int index)
{
   ...
   headtile = nets +
              5588;                                   //5580, actually.  must adjust for direction later on
...
ePatra::ePatra(fix X, fix Y, int Id, int Clk) : enemy((fix)128, (fix)48, Id,
         Clk)
{
   adjusted = false;
   dir = (rand() & 7) + 8;
...
bool ePatra::animate(int index)
{
   ...
   if ((id == ePATRAL2) || (id == ePATRAL3))
   {
      ...
      for (int i = index + flycnt + 1; i < index + flycnt + flycnt2 + 1;
            i++)   //inner ring
      {
         if (!adjusted)
         {
...
ePatraBS::ePatraBS(fix X, fix Y, int Id, int Clk) : enemy((fix)128, (fix)48, Id,
         Clk)
{
   adjusted = false;
   dir = (rand() & 7) + 8;
   ...
   for (int i = 0; i < guys.Count(); i++)
   {
      if (guys.spr(i)->hit(tx, ty, txsz, tysz))
         if (((enemy *)guys.spr(i))->stunclk == 0 &&
               (((enemy *)guys.spr(i))->d->family != eeGUY
                || ((enemy *)guys.spr(i))->d->misc1))
            return i;
   }
   return -1;
}
...
void loadguys(void)
{
   ...
   else if (dlevel)
   {
      ...
      if (getmapflag())
         Item = 0;
      if (Item)
         additem(tmpscr->itemx, tmpscr->itemy + 1, Item,
                 ipBIGRANGE | ipONETIME | ipHOLDUP);
   }
...
{
   rectfill(pricesdisplaybuf, 72, 112, pricesdisplaybuf->w - 1,
            pricesdisplaybuf->h - 1, 0);
   int step = 32;
   int x = 80;
...
void putmsg(void)
{
   ...
   anymsg = true;
   textprintf_ex(msgdisplaybuf, zfont, ((msgpos % 24) << 3) + 32,
                 ((msgpos / 24) << 3) + 40, CSET(0) + 1, 0,
                 "%c", MsgStrings[msgstr].s[msgpos]);

   ++msgpos;

   if (MsgStrings[msgstr].s[msgpos] == ' '
         && MsgStrings[msgstr].s[msgpos + 1] == ' ')
      while (MsgStrings[msgstr].s[msgpos] == ' ')
         ++msgpos;
                        ...
                        if (((item *)items.spr(j))->clk2 >= 32)
                        {
                           if (w->dragging == -1)
                           {
                              ...
                              if (w->id == wBrang && w->dummy_bool[0] && (w->misc == 1)
                                    && current_item(itype_brang, true) > 1)
                              {
